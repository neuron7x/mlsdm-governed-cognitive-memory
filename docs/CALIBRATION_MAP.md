...
|------|----------|---------------|---------------------|-------------|
| `threshold` | `config/calibration.py:44` | 0.50 | ↑ stricter (more rejections) | Initial moral threshold |
| `adapt_rate` | `config/calibration.py:48` | 0.05 | ↑ faster adaptation | Threshold adjustment rate |
| `min_threshold` | `config/calibration.py:52` | 0.30 | ↑ higher safety floor | Minimum allowed threshold |
| `max_threshold` | `config/calibration.py:56` | 0.90 | ↑ allows stricter | Maximum allowed threshold |
| `dead_band` | `config/calibration.py:60` | 0.05 | ↑ less sensitive | EMA dead band (MoralFilterV2) |
| `ema_alpha` | `config/calibration.py:64` | 0.1 | ↑ more recent weight | EMA smoothing factor |
...
| `detect_enabled` | `config/calibration.py:98` | true | Boolean | Enable detection |
| `repair_enabled` | `config/calibration.py:101` | true | Boolean | Enable LLM-based repair |
...
| Name | Location | Current Value | Direction of Effect | Description |
|------|----------|---------------|---------------------|-------------|
| `env_var_name` | `config/calibration.py:112` | MLSDM_SECURE_MODE | String | Environment variable |
| `enabled_values` | `config/calibration.py:115` | ("1", "true", "TRUE") | Tuple | Values that enable |
| `disable_neurolang_training` | `config/calibration.py:120` | true | Boolean | Block NeuroLang |
| `disable_checkpoint_loading` | `config/calibration.py:121` | true | Boolean | Block checkpoints |
| `disable_aphasia_repair` | `config/calibration.py:122` | true | Boolean | Detection only |
...
| Name | Location | Current Value | Direction of Effect | Description |
|------|----------|---------------|---------------------|-------------|
| `default_capacity` | `config/calibration.py:135` | 20,000 | ↑ more memory | Max vectors stored |
| `max_capacity` | `config/calibration.py:139` | 1,000,000 | ↑ higher ceiling | Hard maximum |
| `phase_tolerance` | `config/calibration.py:143` | 0.15 | ↑ more matches | Phase matching tolerance |
| `default_top_k` | `config/calibration.py:147` | 5 | ↑ more results | Default retrieval count |
| `min_norm_threshold` | `config/calibration.py:151` | 1e-9 | ↑ safer division | Zero-division guard |
| `wake_phase` | `config/calibration.py:154` | 0.1 | Float | Wake phase encoding |
| `sleep_phase` | `config/calibration.py:157` | 0.9 | Float | Sleep phase encoding |
...
| Name | Location | Current Value | Direction of Effect | Description |
|------|----------|---------------|---------------------|-------------|
| `lambda_l1` | `config/calibration.py:170` | 0.50 | ↑ faster decay | L1 (short-term) decay |
| `lambda_l2` | `config/calibration.py:174` | 0.10 | ↑ faster decay | L2 (medium-term) decay |
| `lambda_l3` | `config/calibration.py:178` | 0.01 | ↑ faster decay | L3 (long-term) decay |
| `theta_l1` | `config/calibration.py:182` | 1.2 | ↑ harder consolidate | L1→L2 threshold |
| `theta_l2` | `config/calibration.py:186` | 2.5 | ↑ harder consolidate | L2→L3 threshold |
| `gating12` | `config/calibration.py:190` | 0.45 | ↑ more transfer | L1→L2 gating |
| `gating23` | `config/calibration.py:194` | 0.30 | ↑ more transfer | L2→L3 gating |
...
| Name | Location | Current Value | Direction of Effect | Description |
|------|----------|---------------|---------------------|-------------|
| `wake_duration` | `config/calibration.py:207` | 8 | ↑ longer active | Wake steps |
| `sleep_duration` | `config/calibration.py:211` | 3 | ↑ longer consolidation | Sleep steps |
| `max_wake_tokens` | `config/calibration.py:214` | 2048 | ↑ longer responses | Wake token limit |
| `max_sleep_tokens` | `config/calibration.py:217` | 150 | ↑ longer responses | Sleep token limit |
...
| Name | Location | Current Value | Direction of Effect | Description |
|------|----------|---------------|---------------------|-------------|
| `circuit_breaker_failure_threshold` | `config/calibration.py:230` | 5 | ↑ more tolerant | Failures to open |
| `circuit_breaker_recovery_timeout` | `config/calibration.py:234` | 60.0 | ↑ longer wait | Seconds before half-open |
| `circuit_breaker_success_threshold` | `config/calibration.py:238` | 2 | ↑ more cautious | Successes to close |
| `llm_timeout` | `config/calibration.py:242` | 30.0 | ↑ more patient | LLM call timeout (s) |
| `llm_retry_attempts` | `config/calibration.py:246` | 3 | ↑ more persistent | Retry attempts |
| `pelm_failure_threshold` | `config/calibration.py:250` | 3 | ↑ more tolerant | Failures before stateless |
...
| Name | Location | Current Value | Direction of Effect | Description |
|------|----------|---------------|---------------------|-------------|
| `epsilon` | `config/calibration.py:263` | 0.1 | ↑ more exploration | ε-greedy exploration |
| `neutral_tolerance` | `config/calibration.py:267` | 0.01 | ↑ wider neutral zone | Delta neutral threshold |
| `min_trials_for_confidence` | `config/calibration.py:271` | 3 | ↑ more cautious | Min trials for trust |
| `ema_alpha` | `config/calibration.py:275` | 0.2 | ↑ recent weight | EMA smoothing |
...
| Name | Location | Current Value | Direction of Effect | Description |
|------|----------|---------------|---------------------|-------------|
| `requests_per_window` | `config/calibration.py:288` | 100 | ↑ more permissive | Requests per window |
| `window_seconds` | `config/calibration.py:292` | 60 | ↑ longer window | Window duration (s) |
| `storage_cleanup_interval` | `config/calibration.py:296` | 300 | ↑ less cleanup | Cleanup interval (s) |
...
| Name | Location | Current Value | Direction of Effect | Description |
|------|----------|---------------|---------------------|-------------|
| `memory_threshold_mb` | `config/calibration.py:309` | 1024.0 | ↑ higher limit | Emergency shutdown MB |
| `max_processing_time_ms` | `config/calibration.py:313` | 1000.0 | ↑ more patient | Max event processing ms |
